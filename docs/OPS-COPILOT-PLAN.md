# Lane B (PARKED): Ops Assistant — operator-side agent for the command post
...
- NPU contention: sim floods must use pre-cached triage results so the Assistant never queues behind 30-45s of serialized triage.

## Build order (~7-9h)

1. **(1.5h) Deterministic spine** — new `command-post/Assistant.py`: keyword fast-path for the rehearsed utterances → direct `Store` calls; `POST /Assistant` in app.py; new WS card kinds.
...
6. **(1h) Go/no-go bench** — 20 canned utterances (`bench.py` pattern) on the X Elite: if tool-JSON success after one retry < ~90%, ship regex-only Assistant; LLM writes prose only.
