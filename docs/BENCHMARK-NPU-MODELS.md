...
- `mesh-app/.../chat/AssistantModels.kt` — Sahayak added as the first/default catalogue entry (non-gated).
- `mesh-app/.../chat/ChatViewModel.kt` — auto-selects a side-loaded `sahayak*` GGUF at bootstrap so it
  loads straight onto the NPU with no download step.
- `mesh-app/.../chat/ModelPrepViewModel.kt` — recommends Sahayak on every device (the fine-tune beats
