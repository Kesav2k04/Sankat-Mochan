...
Arduino UNO Q is fully self-contained. The Pi keeps its own copy in `pi-code/`.
...
#    Point it at this arduino-unoq/ folder (its app.yaml + sketch/ is the modem):
arduino-app-cli app start ~/arduino-unoq
...
`run.sh` builds a venv in `arduino-unoq/.venv`, installs `bleak` + `requests` +
